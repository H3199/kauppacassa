...
def comment(user_id, chain_id, comment):
    ...
    query_comments = (
                "INSERT INTO comments (comment_id, chain_id, user_id, timestamp, comment) VALUES (" + comment_id + ", " + chain_id + ", " + user_id + ", \'" + now + "\', \'" + comment + "\')")
    query_chains = (
                "INSERT INTO com_chain12 (chain_id, comment_id, timestamp) VALUES (" + chain_id + ", " + comment_id + ", \'" + now + "\')")
    query_karma = ("UPDATE comment_karma SET karma = karma + 0 WHERE comment_id = " + comment_id)
    query_user_comment_history = (
                "INSERT INTO user_comment_history (user_id, comment_id, timestamp) VALUES (" + user_id + ", " + comment_id + ", \'" + now + "\')")
...
def showUserKarma(user_id):
    ...
    comment_history = session.execute(comment_history_query)
    for comment in comment_history:
        comment_id = comment.comment_id
        karma_query = ("SELECT karma from comment_karma WHERE comment_id = " + str(comment_id))
        karma_rows = session.execute(karma_query)
...
def createChain(chain_name, board_id):
    ...
    query_chains = (
                "INSERT INTO com_chain12 (chain_id, timestamp, chain_name, board_id) VALUES (" + chain_id + ", \'" + now + "\', \'" + chain_name + "\', " + board_id + ")")
    query_boards = (
...
def createBoard(board_name, parent_id=None, board_level=-1):
    ...
    if parent_id:
        query_boards = (
                    "INSERT INTO com_boards (board_id, timestamp, board_name, parent_board_id) VALUES (" + board_id + ", \'" + now + "\', \'" + board_name + "\', " + parent_id + ")")
        # Parent knows its children:
        update_parent = (
                    "INSERT INTO board_children (board_id, child_id, board_level) VALUES (" + parent_id + ", " + board_id + ", " + str(
                board_level) + ")")
        # Child knows its parent:
        update_child = (
                    "INSERT INTO board_parent (board_id, parent_id, board_level) VALUES (" + board_id + ", " + parent_id + ", " + str(
                board_level) + ")")
        ...
        query_boards = (
                    "INSERT INTO com_boards (board_id, timestamp, board_name) VALUES (" + board_id + ", \'" + now + "\', \'" + board_name + "\')")
    session.execute(query_boards)
